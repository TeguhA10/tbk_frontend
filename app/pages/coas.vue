<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/80 p-5 sm:p-6 rounded-3xl border border-slate-800 shadow-xl">
      <div class="flex items-center gap-3.5">
        ...
        <div>
          ...
          <p class="text-xs text-slate-400 mt-0.5">Daftar kode akun keuangan beserta pemetaan grup kategori dan klasifikasinya</p>
        </div>
      </div>
          ...
          <table class="w-full text-left text-xs">
            ...
            <tbody class="divide-y divide-slate-800/60 font-medium">
              <tr v-for="coa in filteredCoas" :key="coa.id" class="hover:bg-slate-800/40 transition group">
                ...
                <td class="py-4 px-6">
                  <span class="px-3 py-1 rounded-xl bg-slate-800/80 text-slate-200 font-medium text-xs border border-slate-700/60">
                    {{ coa.category?.name || '-' }}
                  </span>
                </td>
                <td class="py-4 px-6">
                  ...
                  >
                    <component :is="coa.category?.type === 'income' ? ArrowDownRight : ArrowUpRight" class="w-3.5 h-3.5" />
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
        <div class="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6">
          ...
          <form @submit.prevent="saveCoa" class="space-y-4">
            <div>
              <label class="block text-xs font-semibold text-slate-300 mb-1.5">Kode COA <span class="text-rose-400">*</span></label>
              <input 
                v-model="form.code" 
                type="text" 
                required
                placeholder="Misal: 404" 
            ...
            <div>
              <label class="block text-xs font-semibold text-slate-300 mb-1.5">Nama Akun COA <span class="text-rose-400">*</span></label>
              <input 
                v-model="form.name" 
                type="text" 
                required
                placeholder="Misal: Bonus Kinerja / Sewa Bangunan" 
            ...
            <div>
              <label class="block text-xs font-semibold text-slate-300 mb-1.5">Kategori Master <span class="text-rose-400">*</span></label>
              <select 
    ...
    <AppConfirmModal 
      ...
      :item-name="coaToDelete ? `${coaToDelete.code} - ${coaToDelete.name}` : ''"
      confirm-text="Ya, Hapus COA"
      :loading="deleting"
