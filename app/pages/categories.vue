<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/80 p-5 sm:p-6 rounded-3xl border border-slate-800 shadow-xl">
      <div class="flex items-center gap-3.5">
        ...
        <div>
          ...
          <p class="text-xs text-slate-400 mt-0.5">Kelola pengelompokan akun pemasukan (Income) dan pengelompokan biaya operasional (Expense)</p>
        </div>
      </div>
    ...
    <!-- Search & Type Filter Tabs Toolbar -->
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
    ...
    <div class="bg-slate-900/80 rounded-3xl border border-slate-800 shadow-xl overflow-hidden">
      ...
      <!-- Empty state -->
      <div v-else-if="filteredCategories.length === 0" class="p-16 text-center text-slate-400 text-xs flex flex-col items-center justify-center gap-3">
              ...
              <tr v-for="cat in filteredCategories" :key="cat.id" class="hover:bg-slate-800/40 transition group">
                ...
                <td class="py-4 px-6 font-bold text-white text-sm">
                  <div class="flex items-center gap-2.5">
                    <span :class="['w-2 h-2 rounded-full', cat.type === 'income' ? 'bg-emerald-400' : 'bg-rose-400']"></span>
                    <span>{{ cat.name }}</span>
                  </div>
                </td>
                <td class="py-4 px-6">
                ...
                <td class="py-4 px-6 text-center">
                  <span class="px-3 py-1 rounded-xl bg-slate-800/80 text-slate-300 font-mono text-xs border border-slate-700/60">
        ...
        <div class="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6">
          <div class="flex items-center justify-between border-b border-slate-800 pb-4">
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-base font-extrabold text-white">{{ isEditing ? 'Edit Kategori' : 'Tambah Kategori Baru' }}</h3>
            </div>
          ...
          <form @submit.prevent="saveCategory" class="space-y-4">
            <div>
              <label class="block text-xs font-semibold text-slate-300 mb-1.5">Nama Kategori <span class="text-rose-400">*</span></label>
              <input 
                v-model="form.name" 
                type="text" 
                required
                placeholder="Misal: Operational Expense" 
            ...
            <div>
              <label class="block text-xs font-semibold text-slate-300 mb-2">Tipe Klasifikasi <span class="text-rose-400">*</span></label>
    ...
    <AppConfirmModal 
      ...
      confirm-text="Ya, Hapus Kategori"
      :loading="deleting"
