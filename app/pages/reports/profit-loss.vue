<template>
  <div class="space-y-6 sm:space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/80 p-5 sm:p-7 rounded-3xl border border-slate-800 shadow-xl">
      ...
      <div class="flex flex-wrap items-center gap-3 self-start sm:self-auto">
        ...
        <!-- Excel Export Button -->
        <a 
          :href="`${apiBase}/reports/profit-loss/export`" 
          target="_blank" 
          download
    ...
    <div v-if="reportData" class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
      ...
      <div class="p-5 rounded-3xl bg-gradient-to-br from-slate-900 via-indigo-950/40 to-slate-900 border border-indigo-500/30 space-y-2">
        <div class="flex items-center justify-between">
        ...
        <p :class="['text-xl sm:text-2xl font-extrabold font-mono', overallNet >= 0 ? 'text-emerald-400' : 'text-rose-400']">
          {{ formatRupiah(overallNet) }}
        </p>
      </div>
    </div>

    <!-- Main Report Matrix Card -->
    <div class="bg-slate-900/90 rounded-3xl border border-slate-800 shadow-2xl overflow-hidden">
      <!-- Card Subheader -->
      <div class="p-5 sm:p-6 border-b border-slate-800/80 bg-slate-950/60 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          ...
          <p class="text-xs font-mono text-slate-400 mt-0.5">
            PERIODE LAPORAN: {{ reportData?.months?.[0] || 'Jan 2022' }} s/d {{ reportData?.months?.[reportData?.months?.length - 1] || 'Mar 2022' }}
          </p>
        </div>
      ...
      <!-- Loading State -->
      <div v-if="pending" class="p-20 text-center text-slate-400 text-xs flex flex-col items-center justify-center gap-3 animate-pulse">
        <Loader2 class="w-6 h-6 animate-spin text-indigo-400" />
        <span>Menghitung dan menyusun laporan profit & loss...</span>
      </div>

      <!-- Report Table with Sticky Category Column for Flawless Horizontal Scroll -->
      <div v-else class="overflow-x-auto relative">
        <table class="w-full text-left border-collapse min-w-[700px]">
          <thead>
            <tr class="bg-slate-950/90 text-slate-400 uppercase text-[11px] font-bold tracking-wider border-b border-slate-800">
              <th class="py-4 px-6 sticky left-0 z-20 bg-slate-950/95 backdrop-blur-sm min-w-[220px] border-r border-slate-800/60">
                Kategori Akun
              </th>
          ...
          <tbody class="divide-y divide-slate-800/50 text-xs font-medium">
            <!-- Header Section: INCOME -->
            <tr class="bg-slate-950/80">
              <td :colspan="(reportData?.months?.length || 0) + 1" class="py-3 px-6 text-emerald-400 font-extrabold uppercase text-[11px] tracking-wider border-y border-slate-800 sticky left-0 z-10 bg-slate-950">
                ▸ Income (Pemasukan)
              </td>
            </tr>
            ...
            <tr 
              v-for="incCat in reportData?.income_categories" 
              :key="incCat.id" 
              class="hover:bg-slate-800/40 transition group"
            >
              <td class="py-3.5 px-6 sm:px-8 text-slate-200 font-semibold sticky left-0 z-10 bg-slate-900/95 border-r border-slate-800/60">
            ...
            <tr class="bg-emerald-950/30 border-y-2 border-emerald-500/30 text-emerald-300 font-extrabold text-xs">
              <td class="py-4 px-6 uppercase tracking-wider sticky left-0 z-10 bg-slate-950/95 border-r border-slate-800/60">
                Total Income
              </td>
              <td 
                v-for="month in reportData?.months" 
            ...
            <!-- Header Section: EXPENSE -->
            <tr class="bg-slate-950/80">
              <td :colspan="(reportData?.months?.length || 0) + 1" class="py-3 px-6 text-rose-400 font-extrabold uppercase text-[11px] tracking-wider border-y border-slate-800 sticky left-0 z-10 bg-slate-950">
            ...
            <tr 
              v-for="expCat in reportData?.expense_categories" 
              :key="expCat.id" 
              class="hover:bg-slate-800/40 transition group"
            >
              <td class="py-3.5 px-6 sm:px-8 text-slate-200 font-semibold sticky left-0 z-10 bg-slate-900/95 border-r border-slate-800/60">
            ...
            <!-- NET INCOME ROW -->
            <tr class="bg-gradient-to-r from-indigo-950 via-slate-900 to-emerald-950 border-t-2 border-b-4 border-indigo-500/40 text-white font-extrabold text-sm">
              <td class="py-5 px-6 uppercase tracking-wider text-indigo-300 sticky left-0 z-10 bg-slate-950/95 border-r border-slate-800/60">
