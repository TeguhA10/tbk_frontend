<template>
  <div class="space-y-6">
    <div
      ...
      <div class="flex items-center gap-3.5">
        ...
        <div>
          ...
          <p class="text-xs text-slate-400 mt-0.5">Pencatatan mutasi transaksi debet dan kredit berdasarkan kode akun
            COA</p>
        </div>
      </div>
    ...
    <div class="p-4 sm:p-5 bg-slate-900/60 rounded-3xl border border-slate-800 space-y-4">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="relative">
          ...
          <input v-model="txStore.filters.search" @input="debounceLoad" type="text"
          ...
          <button v-if="txStore.filters.search" @click="txStore.filters.search = ''; txStore.fetchTransactions(1)"
            class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300">
            <X class="w-3.5 h-3.5" />
          </button>
        </div>

        <select v-model="txStore.filters.coa_id" @change="txStore.fetchTransactions(1)"
        ...
        <div class="flex items-center gap-2 sm:col-span-2 lg:col-span-2">
          <div class="relative flex-1">
            <input v-model="txStore.filters.start_date" @change="txStore.fetchTransactions(1)" type="date"
          ...
          <div class="relative flex-1">
            <input v-model="txStore.filters.end_date" @change="txStore.fetchTransactions(1)" type="date"
              class="w-full px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-indigo-500 transition" />
          </div>
          <button @click="txStore.resetFilters()"
      ...
      <div class="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-800/80 text-xs">
        <div class="flex flex-wrap items-center gap-4 sm:gap-6">
          ...
          <div class="flex items-center gap-2">
            ...
            <strong class="text-emerald-400 font-mono font-bold">{{ formatRupiah(txStore.totalFilteredCredit)
              }}</strong>
          </div>
          <div class="flex items-center gap-2">
    ...
    <div class="bg-slate-900/80 rounded-3xl border border-slate-800 shadow-xl overflow-hidden">
      ...
      <div v-else>
        ...
        <div class="md:hidden divide-y divide-slate-800/60 p-3 space-y-3">
          <div v-for="tx in txStore.transactions" :key="tx.id"
        ...
        <div v-if="!txStore.loading && txStore.transactions.length > 0"
          class="p-4 sm:p-5 bg-slate-950/40 border-t border-slate-800/80 flex flex-col md:flex-row items-center justify-between gap-4">
          <div class="flex flex-wrap items-center justify-center sm:justify-start gap-3 text-xs text-slate-400">
            <span>
              Menampilkan <strong class="text-white font-mono font-semibold">{{ txStore.pagination.from }}</strong> -
              <strong class="text-white font-mono font-semibold">{{ txStore.pagination.to }}</strong> dari <strong
                class="text-white font-mono font-semibold">{{ txStore.pagination.total }}</strong> transaksi
            </span>
            <div class="flex items-center gap-2 pl-0 sm:pl-3 border-l-0 sm:border-l border-slate-800">
              ...
              <select v-model.number="txStore.pagination.perPage"
          ...
          <div class="flex items-center gap-1.5 self-center md:self-auto">
            <button @click="txStore.changePage(1)" :disabled="txStore.pagination.currentPage <= 1"
            ...
            <button @click="txStore.changePage(txStore.pagination.currentPage - 1)"
            ...
            <div class="flex items-center gap-1 px-1">
              <template v-for="(p, idx) in txStore.displayedPages" :key="idx">
                ...
                <button v-else @click="txStore.changePage(Number(p))" :class="[
            ...
            <button @click="txStore.changePage(txStore.pagination.lastPage)"
              :disabled="txStore.pagination.currentPage >= txStore.pagination.lastPage"
              class="p-2 rounded-xl border border-slate-800/80 bg-slate-900/80 hover:bg-slate-800 text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition active:scale-95"
        ...
        <div class="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6">
          <div class="flex items-center justify-between border-b border-slate-800 pb-4">
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-base font-extrabold text-white">{{ isEditing ? 'Edit Transaksi' : 'Tambah Transaksi Baru'
                }}
              </h3>
            </div>
          ...
          <form @submit.prevent="saveTransaction" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-semibold text-slate-300 mb-1.5">Tanggal Transaksi <span
              ...
              <div>
                <label class="block text-xs font-semibold text-slate-300 mb-1.5">Pilih Akun COA <span
                    class="text-rose-400">*</span></label>
                <select v-model="form.coa_id" required @change="onCoaChange"
                  class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-indigo-500 transition">
            ...
            <div>
              <template v-if="selectedCategoryType === 'expense'">
                <label class="block text-xs font-semibold text-rose-400 mb-1.5">Debit (Biaya / Pengeluaran) <span
                    class="text-rose-400">*</span></label>
                <input v-model.number="form.debit" type="number" min="0" step="1000" placeholder="0"
              ...
              <template v-else-if="selectedCategoryType === 'income'">
                <label class="block text-xs font-semibold text-emerald-400 mb-1.5">Credit (Pemasukan / Income) <span
                    class="text-rose-400">*</span></label>
                <input v-model.number="form.credit" type="number" min="0" step="1000" placeholder="0"
              ...
              <template v-else>
                <label class="block text-xs font-semibold text-slate-400 mb-1.5">Nominal</label>
                <p class="text-[11px] text-slate-500 px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl">
                  Pilih akun COA terlebih dahulu untuk menentukan Debit / Credit
                </p>
              </template>
            </div>

            <div class="p-3 rounded-2xl bg-slate-950/60 border border-slate-800/80 text-[11px] text-slate-400">
              💡 <em>Tips:</em> Field nominal otomatis mengikuti kategori COA — <strong>expense</strong> hanya mengisi
              Debit, <strong>income</strong> hanya mengisi Credit.
            </div>
    ...
    <AppConfirmModal v-model="showDeleteModal" title="Hapus Catatan Transaksi"
      ...
      confirm-text="Ya, Hapus Transaksi" :loading="deleting" @confirm="executeDelete" />
  </div>
</template>
...
const selectedCoa = computed(() => coaStore.coas.find(c => c.id === form.value.coa_id))
const selectedCategoryType = computed(() => selectedCoa.value?.category?.type || null)

const onCoaChange = () => {
  const selected = coaStore.coas.find(c => c.id === form.value.coa_id)
  const type = selected?.category?.type
  if (type === 'expense') {
    form.value.credit = 0
  } else if (type === 'income') {
    form.value.debit = 0
